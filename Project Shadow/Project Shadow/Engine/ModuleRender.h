...
public:
	...
	void ResetViewPort();
	bool Blit(SDL_Texture* texture, int x, int y, const SDL_Rect* section = NULL, float speed = 1.0f, double angle = 0, bool h_flip = false, int pivot_x = INT_MAX, int pivot_y = INT_MAX) const;
	bool BlitGui(SDL_Texture* texture, int x, int y, const SDL_Rect* section = NULL, bool use_camera = false, float speed = 1.0f, double angle = 0, bool h_flip = false, SDL_Color color = { 255, 255, 255, 255 }, int pivot_x = INT_MAX, int pivot_y = INT_MAX, float selfScale = 1.f);
